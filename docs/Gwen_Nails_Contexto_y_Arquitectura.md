...
Avance posterior del mismo ciclo: checklist visual publicado en `ee562e3`, seis reglas con texto/indicadores verdes y confirmación de coincidencia. Validación del formulario compartida con backend. Chrome local con API simulada aprobó bloqueo de secuencia, condiciones cumplidas, confirmación y un único envío válido con vuelta al login. Antigravity agotó tiempo y el revisor reparó una edición JSX incompleta antes de publicar. No equivale a QA desplegado ni cierra los flujos de provisión/reset/Mi cuenta.

Actualización verificada 2026-09-02 16:25 UTC: política compartida implementada por Antigravity y publicada en `86c5fd6`, rama `antigravity/AUTH-002-session-benefits`. Doce casos del módulo puro aprobados por Codex, con concordancia checklist/validador. No se probaron DB ni navegador con ese módulo. TypeScript global aún falla por `node-fetch` ausente en `scripts/test_booking_concurrency.ts`. Siguiente bloque en ejecución: checklist visual verde y validación del formulario mediante el módulo compartido. AUTH-002 permanece IN_PROGRESS; la provisión de clave temporal, Mi cuenta y pruebas completas siguen pendientes. Los problemas operativos de permisos Git/Antigravity se resolvieron; el runner detecta respuestas vacías como resultado no verificado.

Decisión vigente de contraseñas del propietario: cuentas exclusivas para empleados/administración; email opcional, sin recupero por email para empleados. Username sugerido inicial+apellido normalizado, sufijo ante duplicados. Clave temporal genérica compartida configurada fuera de Git, acceso restringido y cambio obligatorio primer ingreso/reset; administrador restablece olvidadas. Mi cuenta permite cambio con clave actual, nueva y confirmación. Clave personal de 8 a 16 caracteres (reemplaza 12–128 anteriores), mayúscula, minúscula, dígito y símbolo; prohibir secuencias ascendentes/descendentes de cuatro números; no igual usuario/actual/temporal. Checklist textual con condiciones que cambian a verde. Backend autoritativo y política compartida. No invalidar claves existentes al hacer login. Implementar en bloques pequeños y probar ambos lados.
...
Actualización 2026-09-03: revisión manual de AUTH-002, commit 2c43de1. Estado CHANGES_REQUESTED, no integrado/desplegado. Cinco suites aisladas existentes y tsc/build cliente-servidor pasan; prueba adicional del handler confirma que crear empleado con contraseña explícita no obliga a cambiarla. Pendientes: corregir ese flujo, no ocultar fallos HTTP de alta/reset en UI, mostrar username generado y reglas coherentes, sustituir pruebas de lógica duplicada y comprobar protección concurrente del último superadmin. La entrega incorpora Mi cuenta y login transaccional pero no acredita QA navegador ni PostgreSQL real. Ver docs/coordination/reviews/AUTH-002-manual-20260903.md. Automatización PAUSADA por el propietario; trabajo actual manual. No dar por cerrados los puntos 4–6 ni iniciar reservas todavía.
