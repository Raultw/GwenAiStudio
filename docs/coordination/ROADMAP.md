...
7. ⏳ **Confirmación de reserva por email y datos de gestión (BOOK-003).** Mostrar código de reserva y clave de gestión, con aviso explícito para guardarlos o tomar captura, tenga o no email. Email de confirmación con los datos del turno, ambos datos de gestión y enlace directo seguro para gestionar su cancelación. El enlace abre una pantalla de confirmación: nunca cancela con solo abrirlo. Verificar envío a casilla propia autorizada, mantener estética y formatos argentinos. Implementar/validar el destino mínimo del enlace con el punto 12; no enviar enlaces sin destino funcional.
...
- 1def4ab serializa en updateUser la degradación/desactivación del último superadmin; 7 casos nuevos, 66 AUTH-002 totales pasan y tsc limpio. Falta corregir bootstrap con conexión única, PostgreSQL real, integración y QA. Antigravity ya no pidió permisos en el flujo solo-archivos, pero agotó tiempo y su edición truncada fue descartada; problema mitigado, no resuelto globalmente.
- 18fac8d corrige bootstrap PostgreSQL con una sola conexión/transacción y auditoría atómica; 6 casos nuevos, 72 AUTH-002 totales y tsc pasan. Puntos 4–6 continúan sin cierre hasta integración y QA. Flujo estable adoptado: Antigravity propone en tareas pequeñas sin escribir/terminal; Codex aplica, prueba y publica, evitando archivos truncados por timeout.
