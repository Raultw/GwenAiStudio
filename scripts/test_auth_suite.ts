...
// ---------------------------------------------------------------------------
// TEST SUITE RUNNER WITH ISOLATION SETUP AND RELIABLE RESTORATION
// ---------------------------------------------------------------------------
...
async function runAuthSuite() {
  ...
  // 1. Capture original repository root, working directory and server.ts path BEFORE changing cwd
  ...
  // 2. Snapshot original environment variables BEFORE any mutation
  ...
  try {
    // 3. Mutate environment variables for isolation inside protected try block
    ...
    // 4. Create isolated temporary directory in os.tmpdir()
    ...
    const parentDir = path.dirname(resolvedTempDir);
    const baseName = path.basename(resolvedTempDir);

    if (parentDir !== resolvedTmpRoot || !baseName.startsWith('gwen-auth-suite-')) {
      throw new Error(`Security validation failed: Invalid temp directory path ${tempDir}`);
    }
    ...
    console.log('======================================================');
    console.log('🧪 GWEN NAILS - COMPREHENSIVE AUTH TEST SUITE (39 TESTS)');
    console.log('======================================================');
    ...
    // 2. Restore standard environment variables
    try {
      ...
      // 3. Clear any newly set bootstrap env variables and restore original snapshot
      for (const key of Object.keys(process.env)) {
        if (key.startsWith('SUPERADMIN_BOOTSTRAP_')) {
          delete process.env[key];
        }
      ...
      console.error('Error restoring environment variables:', envErr);
      cleanupFailed = true;
    }
    ...
    if (tempDir) {
      try {
        ...
        if (parentDir === resolvedTmpRoot && baseName.startsWith('gwen-auth-suite-')) {
          ...
          console.error(`Safety check failed: tempDir ${resolvedTempDir} does not meet cleanup safety criteria.`);
          cleanupFailed = true;
        }
      } catch (cleanupErr) {
        console.error('Error cleaning temporary directory:', cleanupErr);
        cleanupFailed = true;
      }
    }
  }

  if (suiteError || failed > 0 || cleanupFailed) {
    process.exitCode = 1;
  } else {
    process.exitCode = 0;
  }
...
// The middleware owns a recurring timer. Exit only after all cleanup in finally.
runAuthSuite().then(() => {
