...
function setup(postgres=false){
 ...
 const client={query:async(sql,args)=>{state.queries++;if(state.fail&&String(sql).includes('UPDATE users'))throw Error('sensitive-sql-details');if(String(sql).includes('SELECT id,'))return {rows:[{id:'qa',username:'qa',email:null,rol:'profesional',profesional_id:null,activo:true,nombre:'Keep',must_change_password:false,created_at:'2026-01-01',updated_at:'2026-01-01'}]};if(String(sql).includes('UPDATE users'))return {rows:[{id:'qa',username:'qa',email:null,rol:'profesional',profesional_id:null,activo:true,nombre:args[6]||'Keep',must_change_password:false,created_at:'2026-01-01',updated_at:'2026-01-01'}]};return {rows:[],rowCount:1};},release:()=>{}};
 const context={crypto:{randomUUID:()=> 'test-id'},normalizeEmail:x=>x.toLowerCase(),validatePasswordPolicy:p=>({valid:typeof p==='string'&&p.length>=8,error:'invalid'}),hashPassword:()=>({hash:'test-hash',salt:'test-salt'}),isPostgresConnected:postgres,pgPool:{connect:async()=>client,query:async()=>{state.queries++;if(state.fail)throw Error('sensitive-sql-details');return {rowCount:1};}},memoryDb:{users:[]},bootstrapMutex:Promise.resolve(),saveLocalFileDb:()=>{state.saved++;},getUserById:async id=>context.memoryDb.users.find(u=>u.id===id)||null,isLastActiveSuperadmin:async()=>false,console:{error:()=>{throw Error('Unexpected sensitive error logging');}}};
...
(async()=>{
 ...
 {const {context:c,state:s}=setup();c.memoryDb.users.push({id:'qa',password:'old-plaintext',nombre:'Keep',passwordHash:'old',salt:'old'});await assert.rejects(()=>c.updateUser('qa',{password:'Synthetic!7',mustChangePassword:true}));const u=c.memoryDb.users[0];assert.equal(u.password,'old-plaintext');assert.equal(u.passwordHash,'old');assert.equal(s.saved,0);count++;}
