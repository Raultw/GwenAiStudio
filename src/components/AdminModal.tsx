...
export const AdminModal: React.FC<AdminModalProps> = ({
  ...
  const [blockHour, setBlockHour] = useState<string>('');
  ...
  const loadAdminData = async () => {
    setIsLoading(true);
    try {
      const [aptRes, srvRes, cfgRes, statsRes] = await Promise.all([
        fetch('/api/turnos'),
  ...
  const handleRemoveBlockedDate = async (dateStr: string) => {
    if (!config) return;
    const newBlocked = config.diasBloqueados.filter(d => d !== dateStr);
    try {
      const res = await fetch('/api/config', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ diasBloqueados: newBlocked })
      });
      if (res.ok) {
              ...
              {activeTab === 'bloqueos' && (
                <div className="max-w-3xl mx-auto space-y-6">
                  <div className="bg-white p-6 rounded-3xl border border-[#E8DCD5]">
                    <h4 className="font-serif text-xl font-medium text-[#241E1A] mb-2">
                      Bloquear Días o Franjas Horarias
                    </h4>
                    <p className="text-xs text-[#7A6B62] mb-6">
                      Los días o franjas horarias bloqueadas quedarán inaccesibles en el motor de reservas público.
                    </p>

                    <form onSubmit={handleBlockDate} className="grid grid-cols-1 sm:grid-cols-3 gap-3 items-end">
                      <div>
                        <label className="block text-xs font-medium text-[#4A3E39] mb-1">Fecha a bloquear *</label>
                        <input
                          type="date"
                          required
                          value={blockDate}
                      ...
                      <div>
                        <label className="block text-xs font-medium text-[#4A3E39] mb-1">Hora específica (Opcional)</label>
                        <input
                          type="time"
                          value={blockHour}
                          onChange={(e) => setBlockHour(e.target.value)}
                          placeholder="Todo el día si queda vacío"
                          className="w-full p-2 rounded-xl bg-[#FAF7F2] border border-[#D9C9BF] text-xs text-[#241E1A]"
                        />
                      </div>
                      <button
                        type="submit"
                        className="py-2.5 px-4 rounded-xl bg-[#241E1A] text-white text-xs font-medium hover:bg-[#8E4455] transition-all cursor-pointer"
                      >
                        Bloquear en Agenda
                      </button>
                    </form>
                  ...
                  <div className="bg-white p-6 rounded-3xl border border-[#E8DCD5]">
                    <h5 className="font-serif text-base font-medium text-[#241E1A] mb-4">
                      Días Completos Bloqueados ({config?.diasBloqueados?.length || 0})
                    </h5>

                    {!config?.diasBloqueados || config.diasBloqueados.length === 0 ? (
                      <p className="text-xs text-[#7A6B62]">No hay fechas bloqueadas actualmente.</p>
                    ) : (
                      <div className="flex flex-wrap gap-2">
                        {config.diasBloqueados.map(d => (
                          <div key={d} className="flex items-center gap-2 bg-[#FAF7F2] px-3 py-1.5 rounded-lg border border-[#E8DCD5] text-xs">
                            <span className="font-mono text-[#8E4455]">{d}</span>
                            <button
                              onClick={() => handleRemoveBlockedDate(d)}
                              className="text-[#8C7A70] hover:text-rose-600"
                              title="Desbloquear"
                            >
